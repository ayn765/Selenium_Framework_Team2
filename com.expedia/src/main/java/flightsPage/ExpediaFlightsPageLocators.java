package flightsPage;

import common.BaseAPI;

public class ExpediaFlightsPageLocators extends BaseAPI {

    public static final String WEBELEMTET_TAB_FLIGHTS = "//*[@id='uitk-tabs-button-container']/li[2]/a/span";
    public static final String WEBELEMTET_TAB_ONEWAY = "//span[contains(text(),'One-way')]";
    public static final String WEBELEMENT_DROPDOWN_NUM_TRAVELERS = "#adaptive-menu > a";
    public static final String WEBELEMENT_LINK_AIRLINE_AGE_RULES = "#airline-age-rules";
    public static final String WEBELEMENT_BUTTON_INCREASE_NUM_CHILDREN = "div.uitk-spacing-padding-blockstart-unset > div:nth-child(2) > div > button:nth-child(3)";
    public static final String WEBELEMENT_LINK_TRAVEL_WITH_CHILD = "//strong[contains(text(),'Infant, child, and senior discounts')]";
    public static final String WEBELEMENT_INPUT_LEAVING_FROM = "button[data-stid=location-field-leg1-origin-menu-trigger]";
    public static final String WEBELEMENT_LEAVING_AIRPORT_SELECTION = "div.truncate>span>strong";
    public static final String WEBELEMENT_INPUT_GOING_TO = "button[data-stid=location-field-leg1-destination-menu-trigger]";
    public static final String WEBELEMENT_BUTTON_DEPARTING = "#d1-btn";
    public static final String WEBELEMENT_GOING_TO_AIRPORT_SELECTION = "//strong[contains(text(),'Tampa (TPA - Tampa Intl.)')]";
    public static final String WEBELEMENT_SELECT_DEPARTING_DATE = "//button[@aria-label='May 9, 2021.']";
    public static final String WEBELEMENT_BUTTON_RETURNING = "#d2-btn";
    public static final String WEBELEMENT_SELECT_RETURNING_DATE = "//button[@aria-label='May 21, 2021.']";
    public static final String WEBELEMENT_BUTTON_DATE_DONE = "//button[@data-stid='apply-date-picker']";
    public static final String WEBELEMENT_BUTTON_SUBMIT_FLIGHT_SEARCH = ".uitk-layout-grid-item-columnspan-large-2 >button[type=submit]";
    public static final String WEBELEMENT_MESSAGE_COVID = "p.uitk-experimental-banner-body";
    public static final String WEBELEMENT_CHECKBOX_NON_STOP = "#stops-0";
    public static final String WEBELEMENT_BUTTON_NON_STOP = "span.uitk-pill-text";
    public static final String WEBELEMENT_BUTTON_CHANGE_TRIP = "div.reasons-to-believe.all-b-margin-six > section > div > div > div:nth-child(1) > a";
    public static final String WEBELEMENT_CHAT_WITH_VIRTUAL_AGENT = "h2.uitk-subheading";
    public static final String WEBELEMENT_BUTTON_USE_CREDIT_OR_COUPON = ".all-b-margin-six > section > div > div > div:nth-child(2) > a";
//    public static final String WEBELEMENT_
    public static final String WEBELEMENTS_SELECT_COUNTRY_CODE = "#countryCode>option";
    public static final String WEBELEMENT_INPUT_PHONE_NUM = "input#phoneNumber";
    public static final String WEBELEMENT_BUTTON_GET_APP = "#submitBtn";
    public static final String WEBELEMENT_MESSAGE_ERROR_PHONE_NUM = "#phoneNumber-error";
    public static final String WEBELEMENTS_LINKS_FIND_YOUR_NEXT_DEST = "li.uitk-carousel-item>div>div>div>div>a";
    public static final String WEBELEMENTS_LINKS_SWITCH_ROUTINE = ".uitk-card-aloha-content-section.uitk-card-aloha-content-section-has-link>a";
    public static final String WEBELEMENT_BUTTON_HELP = ".uitk-button-floating.launch-button";
    public static final String WEBELEMENT_BUTTON_FLIGHT_CLASS = "a#preferred-class-input-trigger>svg.uitk-icon.uitk-icon-medium";
    public static final String WEBELEMENTS_DROPDOWN_FLIGHT_CLASS = "div.uitk-menu-container-text-nowrap>div>a";
    public static final String WEBELEMENT_LINKS_FOOTER = ".uitk-flex-align-items-center.uitk-flex-wrap.uitk-flex-row>li>a";

}
